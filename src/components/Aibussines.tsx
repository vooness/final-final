...
const Aibussines: React.FC = () => {
  ...
  // Track whether to show particles (desktop only)
  const [enableParticles, setEnableParticles] = useState(false);

  const bgRef = useRef<HTMLDivElement | null>(null);

  // Detect screen size to set isMobile & enableParticles
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      setEnableParticles(width >= 768); // Only show particles on >= 768px
    };
  ...
  // Three.js Particle Setup (desktop only)
  useEffect(() => {
    if (!enableParticles) return; // Skip if we decided not to show
  ...
  return (
    ...
    >
      ...
      {/* Additional Background (only if not mobile) */}
      {!isMobile && (
        <motion.div
          className="absolute inset-0 pointer-events-none z-0"
          initial={{ opacity: 0 }}
