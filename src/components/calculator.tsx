...
export default function CalculatorAI() {
  ...
  // We'll only init Three.js on screens >= 768px wide, for example
  const [enableParticles, setEnableParticles] = useState<boolean>(false);

  const bgRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    // 1) Check if we have window to measure width
    if (typeof window !== "undefined") {
      // 2) If screen width is >= 768 (you can adjust), we enable particles
  ...
  // No custom animation to avoid type issues
  const options = {
    responsive: true,
    maintainAspectRatio: false,
    devicePixelRatio: 1, // <- Tady je nastavení devicePixelRatio
    plugins: {
      legend: {
        labels: {
          color: "#ffffff",
  ...
  return (
    <section className="relative w-full bg-gray-900 py-10 overflow-hidden">
      {/* Only add the ref container if we might show particles */}
      {enableParticles && (
        <div
          ref={bgRef}
